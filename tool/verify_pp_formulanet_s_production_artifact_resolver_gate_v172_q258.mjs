...
const phase = 'V172-Q258-PP-FORMULANET-S-PRODUCTION-ARTIFACT-RESOLVER-GATE';
const sourcePhase = 'V172-Q257-PP-FORMULANET-S-DOWNLOAD-ORCHESTRATOR-UI-GUARD';
const fail = (message) => { console.error(`PP_FORMULANET_S_PRODUCTION_ARTIFACT_RESOLVER_GATE_Q258_BLOCKED: ${message}`); process.exit(1); };
...
for (const path of [
  'lib/features/camera/gauss_pp_formulanet_s_production_artifact_resolver_gate_policy.dart',
  'lib/features/camera/gauss_pp_formulanet_s_production_artifact_resolver_gate.dart',
  ...
  'docs/audit/V172_Q258_PP_FORMULANET_S_PRODUCTION_ARTIFACT_RESOLVER_GATE_AUDIT.md',
  'docs/audit/V172_Q258_CHANGED_FILES.md',
]) if (!exists(path)) fail(`required Q258 file missing: ${path}`);

const manifest = JSON.parse(read('assets/mathlive/manifest.json'));
const policy = read('lib/features/camera/gauss_pp_formulanet_s_production_artifact_resolver_gate_policy.dart');
const gate = read('lib/features/camera/gauss_pp_formulanet_s_production_artifact_resolver_gate.dart');
const test = read('test/v172_q258_pp_formulanet_s_production_artifact_resolver_gate_test.dart');
const audit = read('docs/audit/V172_Q258_PP_FORMULANET_S_PRODUCTION_ARTIFACT_RESOLVER_GATE_AUDIT.md');
const changed = read('docs/audit/V172_Q258_CHANGED_FILES.md');
...
for (const [key, expected] of Object.entries({
  ...
  expectedProductionSha256LockedInQ258: false,
  ...
  if (q258[key] !== expected) fail(`manifest Q258 ${key} expected ${expected} got ${q258[key]}`);
}
if (!Array.isArray(q258.requiredResolverFields) || q258.requiredResolverFields.length < 13) fail('Q258 must define resolver manifest fields');
for (const marker of ['productionArtifactUrlCandidate','expectedSha256Candidate','checksumMismatchBehavior','downloadDisabledByDefault']) if (!q258.requiredResolverFields.includes(marker)) fail(`Q258 missing resolver field ${marker}`);
if (!Array.isArray(q258.requiredResolverSafetyGates) || q258.requiredResolverSafetyGates.length < 13) fail('Q258 must define resolver safety gates');
for (const marker of ['urlCannotEnableDownloadWithoutExpectedSha','expectedShaMustBe64Hex','checksumMismatchBlocksReadyState','editableMathLiveReviewRequired']) if (!q258.requiredResolverSafetyGates.includes(marker)) fail(`Q258 missing resolver gate ${marker}`);
if (!Array.isArray(q258.blockedUntilRealEvidence) || q258.blockedUntilRealEvidence.length < 8) fail('Q258 blockedUntilRealEvidence incomplete');
for (const marker of ['officialArtifactUrlResolved','expectedProductionSha256Locked','androidRuntimeCompatibilityConfirmed','checksumMismatchRejectionProven']) if (!q258.blockedUntilRealEvidence.includes(marker)) fail(`Q258 missing blocked evidence ${marker}`);
if (!Array.isArray(q258.forbiddenActions) || q258.forbiddenActions.length < 18) fail('Q258 forbidden actions incomplete');
for (const marker of ['bindProductionArtifactUrlInQ258','enableProductionDownloadInQ258','runPpFormulaNetSInference','setWorkspaceLatexWithoutUserApproval','claimOcrPass']) if (!q258.forbiddenActions.includes(marker)) fail(`Q258 forbidden action missing ${marker}`);

for (const marker of [phase, 'production-artifact-resolver-gate-only-no-production-download', 'urlCannotEnableDownloadWithoutExpectedSha', 'expectedShaMustBe64Hex', 'checksumMismatchBlocksReadyState', 'editable MathLive review']) {
  if (!policy.includes(marker) && !gate.includes(marker) && !audit.includes(marker) && !readme.includes(marker)) fail(`missing Q258 marker: ${marker}`);
}
for (const marker of ['canResolveProductionArtifactUrl => false','canLockExpectedProductionSha256 => false','canEnableProductionDownloadInQ258 => false','canClaimModelDownloadPass => false','canClaimOcrPass => false','canRunPpFormulaNetSInference => false','canImportCameraOutputWithoutUserApproval => false']) if (!gate.includes(marker)) fail(`gate missing false-claim marker: ${marker}`);
for (const marker of ['blockedMissingQ257OrchestratorGate','blockedInvalidShaCandidate','blockedUnsafeUrlOrRuntimeBinding','productionArtifactResolverGateStaticReady']) if (!gate.includes(marker) || !test.includes(marker)) fail(`gate/test missing decision marker: ${marker}`);
...
for (const forbidden of ['camera_android_camerax','google_mlkit_text_recognition','paddleocr','paddle_flutter','onnxruntime','tesseract','pix2text','flutter_downloader','background_downloader','dio']) {
  if (pubspec.toLowerCase().includes(forbidden)) fail(`Q258 must not add dependency: ${forbidden}`);
}
if (!androidManifest.includes('android.permission.CAMERA')) fail('CAMERA permission must remain');
if (androidManifest.includes('READ_MEDIA_IMAGES')) fail('Q258 must not add media permissions');
for (const forbidden of ['FormulaRecognition(', 'runPpFormulaNetSInference', 'setWorkspaceLatexWithoutUserApproval', 'downloadPpFormulaNetSModel', 'bindProductionArtifactUrlInQ258', 'enableProductionDownloadInQ258']) {
...
const protectedHashes = {
  ...
  "lib/features/camera/gauss_pp_formulanet_s_download_orchestrator_ui_guard_policy.dart": "46031adbaca57322e4a44efa16e52144edfbd9d3ccf8da3a616f6628cd9a4830"
};
...
console.log(JSON.stringify({
  phase,
  status: 'PP_FORMULANET_S_PRODUCTION_ARTIFACT_RESOLVER_GATE_Q258_STATIC_READY_BUT_REAL_ARTIFACT_URL_SHA_DOWNLOADER_RUNTIME_AND_FLUTTER_RETEST_REQUIRED',
  sourcePhase,
  selectedEngineLabel: q258.selectedEngineLabel,
  resolverMode: q258.resolverMode,
  requiredResolverFields: q258.requiredResolverFields.length,
  requiredResolverSafetyGates: q258.requiredResolverSafetyGates.length,
  blockedUntilRealEvidence: q258.blockedUntilRealEvidence.length,
  expectedProductionSha256LockedInQ258: q258.expectedProductionSha256LockedInQ258,
  productionDownloadEnabled: q258.productionDownloadEnabled,
  productionModelUrlBound: q258.productionModelUrlBound,
  realNetworkDownloadWorkerImplemented: q258.realNetworkDownloadWorkerImplemented,
  realOcrRuntimeAdded: q258.realOcrRuntimeAdded,
  paddleRuntimeAdded: q258.paddleRuntimeAdded,
  modelBinaryBundledInBaseApp: q258.modelBinaryBundledInBaseApp,
  productionInferenceAllowed: q258.productionInferenceAllowed,
