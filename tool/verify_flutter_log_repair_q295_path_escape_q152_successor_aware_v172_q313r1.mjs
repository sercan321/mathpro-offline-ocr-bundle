...
const phase = 'V172-Q313R1-FLUTTER-LOG-REPAIR-Q295-PATH-ESCAPE-Q152-SUCCESSOR-AWARE';
const sourcePhase = 'V172-Q313-FULL-CAMERA-OCR-REAL-DEVICE-REGRESSION-GATE';
const successorPhase = 'V172-Q313R2-FLUTTER-ANALYZE-INFO-HYGIENE';
...
if (manifest.cameraOcrLatestPhase === q319SuccessorPhase && !manifest.v172Q319VerifiedArtifactLoadSmokeOnDevice) fail('Q319 successor envelope missing');
if ([q320SuccessorPhase, q321SuccessorPhase, q322SuccessorPhase, q323SuccessorPhase, q324SuccessorPhase, q325SuccessorPhase, q326SuccessorPhase, q327SuccessorPhase, q328SuccessorPhase, q329SuccessorPhase, q330SuccessorPhase, q331SuccessorPhase, q332SuccessorPhase, q333SuccessorPhase, q334SuccessorPhase, q335SuccessorPhase, q336SuccessorPhase, q337SuccessorPhase, q338SuccessorPhase, q339SuccessorPhase, q340SuccessorPhase, q341SuccessorPhase, q342SuccessorPhase].includes(manifest.cameraOcrLatestPhase) && !manifest.v172Q320RuntimeStartupSmokeOnDevice) fail('Q320 successor envelope missing');
...
if (q313r1.phase !== phase) fail('Q313R1 phase mismatch');
if (q313r1.sourcePhase !== sourcePhase) fail('Q313R1 source phase mismatch');
for (const key of ['q295WindowsPathEscapeRepaired','q152ManifestTestSuccessorAware','analyzerConstHygieneApplied']) {
  if (q313r1[key] !== true) fail(`${key} must be true`);
}
for (const key of ['runtimeBehaviorChanged','cameraOcrRuntimeAdded','paddleRuntimeAdded','paddleOcrDependencyAdded','jniNativeHandlerAdded','methodChannelBindingAdded','modelBinaryBundledInBaseApp','productionDownloadEnabled','realImageToLatexInferenceExecuted','editableMathLiveReviewOpened','workspaceImportExecuted','ocrPassClaimed','cameraOcrRuntimePassClaimed','storeReadyPassClaimed','releasePassClaimed']) {
  if (q313r1[key] !== false) fail(`${key} must be false`);
}
for (const file of [
  'lib/features/camera/gauss_flutter_log_repair_q295_path_escape_q152_successor_aware_policy.dart',
  'lib/features/camera/gauss_flutter_log_repair_q295_path_escape_q152_successor_aware.dart',
  ...
  'docs/audit/V172_Q313R1_FLUTTER_LOG_REPAIR_Q295_PATH_ESCAPE_Q152_SUCCESSOR_AWARE_AUDIT.md',
  'docs/audit/V172_Q313R1_CHANGED_FILES.md'
]) { if (!fs.existsSync(file)) fail(`Q313R1 file missing: ${file}`); }
const q295 = read('lib/features/camera/gauss_first_real_image_to_latex_inference.dart');
if (q295.includes("contains('\\files\\models\\')") || q295.includes("contains('\\cache\\crops\\')")) fail('Q295 malformed Windows path string literal still present');
if (!q295.includes("replaceAll('\\\\', '/')")) fail('Q295 path normalization repair missing');
if (!q295.includes('normalizedVerifiedPrivateArtifactPath') || !q295.includes('normalizedCroppedImagePath')) fail('Q295 normalized path getters missing');
const q152Test = read('test/v172_q152_mathlive_final_premium_package_freeze_test.dart');
if (q152Test.includes("expect(manifest['activePhase'], 'V172-Q152-MATHLIVE-FINAL-PREMIUM-PACKAGE-FREEZE')")) fail('Q152 test still expects Q152 activePhase forever');
if (!q152Test.includes('successor phases') || !q152Test.includes("startsWith('V172-Q')")) fail('Q152 successor-aware test marker missing');
const readme = read('README.md');
if (!readme.includes('V172-Q313R1')) fail('README must mention Q313R1');
if ([successorPhase, successorPhaseQ313R3, successorPhaseQ314, successorPhaseQ315, q315R1SuccessorPhase, q316SuccessorPhase, q317SuccessorPhase, q318SuccessorPhase, q319SuccessorPhase, q320SuccessorPhase, q321SuccessorPhase, q322SuccessorPhase, q323SuccessorPhase, q324SuccessorPhase, q325SuccessorPhase, q326SuccessorPhase, q327SuccessorPhase, q328SuccessorPhase, q329SuccessorPhase, q330SuccessorPhase, q331SuccessorPhase, q332SuccessorPhase, q333SuccessorPhase, q334SuccessorPhase, q335SuccessorPhase, q336SuccessorPhase, q337SuccessorPhase, q338SuccessorPhase, q339SuccessorPhase, q340SuccessorPhase, q341SuccessorPhase, q342SuccessorPhase].includes(manifest.cameraOcrLatestPhase) && !readme.includes('V172-Q313R2')) fail('README must mention Q313R2 successor');
...
const changed = read('docs/audit/V172_Q313R1_CHANGED_FILES.md');
if (!changed.includes('gauss_first_real_image_to_latex_inference.dart') || !changed.includes('v172_q152_mathlive_final_premium_package_freeze_test.dart')) fail('Q313R1 changed files must disclose repairs');
const q313Verifier = read('tool/verify_full_camera_ocr_real_device_regression_gate_v172_q313.mjs');
if (!q313Verifier.includes(phase)) fail('Q313 verifier must be Q313R1 successor-aware');
const q312Verifier = read('tool/verify_approved_ocr_workspace_import_runtime_binding_gate_v172_q312.mjs');
if (!q312Verifier.includes(phase)) fail('Q312 verifier must be Q313R1 successor-aware');
...
for (const file of walk('.')) {
  ...
  if (//.test(normalized)) fail('unexpected control char');
  if (//.test(normalized)) fail('unexpected control char');
}
...
console.log(JSON.stringify({ phase, status: 'FLUTTER_LOG_REPAIR_STATIC_READY_RUNTIME_UNCHANGED', sourcePhase, q295WindowsPathEscapeRepaired: q313r1.q295WindowsPathEscapeRepaired, q152ManifestTestSuccessorAware: q313r1.q152ManifestTestSuccessorAware, ocrPassClaimed: q313r1.ocrPassClaimed }, null, 2));
