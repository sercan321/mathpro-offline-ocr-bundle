...
const phase = 'V172-Q263-PP-FORMULANET-S-RUNTIME-PROTOTYPE-SMOKE-EVIDENCE-TEMPLATE';
const sourcePhase = 'V172-Q262-PP-FORMULANET-S-RUNTIME-PROTOTYPE-BRIDGE-GATE';
const fail = (message) => { console.error(`PP_FORMULANET_S_RUNTIME_PROTOTYPE_SMOKE_EVIDENCE_TEMPLATE_Q263_BLOCKED: ${message}`); process.exit(1); };
...
]) if (!exists(path)) fail(`required Q263 file missing: ${path}`);
...
const policy = read('lib/features/camera/gauss_pp_formulanet_s_runtime_prototype_smoke_evidence_template_policy.dart');
const gate = read('lib/features/camera/gauss_pp_formulanet_s_runtime_prototype_smoke_evidence_template.dart');
const test = read('test/v172_q263_pp_formulanet_s_runtime_prototype_smoke_evidence_template_test.dart');
const audit = read('docs/audit/V172_Q263_PP_FORMULANET_S_RUNTIME_PROTOTYPE_SMOKE_EVIDENCE_TEMPLATE_AUDIT.md');
const changed = read('docs/audit/V172_Q263_CHANGED_FILES.md');
const readme = read('README.md');

if (![phase, 'V172-Q264-PP-FORMULANET-S-RUNTIME-PROTOTYPE-EVIDENCE-INTAKE-GATE', 'V172-Q265-PP-FORMULANET-S-RUNTIME-EVIDENCE-ACCEPTANCE-COURT', 'V172-Q266-PP-FORMULANET-S-RUNTIME-ACTIVATION-DECISION-GATE', 'V172-Q267-PP-FORMULANET-S-RUNTIME-INTEGRATION-PREFLIGHT-PACKAGE', 'V172-Q268-PP-FORMULANET-S-ISOLATED-RUNTIME-IMPLEMENTATION-GATE', 'V172-Q269-PP-FORMULANET-S-RUNTIME-DEPENDENCY-ISOLATION', 'V172-Q270-PP-FORMULANET-S-NATIVE-RUNTIME-BRIDGE-SKELETON', 'V172-Q271-PP-FORMULANET-S-PRIVATE-ARTIFACT-LOAD-PROTOTYPE', 'V172-Q272-PP-FORMULANET-S-FIRST-RUNTIME-SMOKE-PROTOTYPE', 'V172-Q273-PP-FORMULANET-S-IMAGE-TO-LATEX-PROTOTYPE-GATE', 'V172-Q274-OCR-RESULT-MATHLIVE-REAL-REVIEW-BINDING', 'V172-Q275-APPROVED-OCR-WORKSPACE-IMPORT-GATE', 'V172-Q276-CAMERA-OCR-REAL-END-TO-END-DEVICE-COURT', 'V172-Q277-CAMERA-OCR-RUNTIME-PREMIUM-REGRESSION'].includes(manifest.cameraOcrLatestPhase)) fail('manifest cameraOcrLatestPhase must point to this phase or successor through Q277');
if (manifest.ppFormulaNetSRuntimePrototypeSmokeEvidenceTemplateLatestPhase !== phase) fail('manifest missing Q263 latest phase');
const q263 = manifest.v172Q263PpFormulaNetSRuntimePrototypeSmokeEvidenceTemplate;
if (!q263 || q263.phase !== phase) fail('manifest missing Q263 envelope');
if (q263.sourcePhase !== sourcePhase) fail('Q263 sourcePhase mismatch');
if (q263.selectedEngineLabel !== 'PP-FormulaNet-S') fail('Q263 selected engine must be PP-FormulaNet-S');
if (q263.fallbackEngineLabel !== 'PP-FormulaNet_plus-L') fail('Q263 fallback must remain PP-FormulaNet_plus-L');
if (q263.smokeEvidenceMode !== 'runtime-prototype-smoke-evidence-template-only-no-runtime-no-inference') fail('Q263 smoke evidence mode mismatch');
for (const [key, expected] of Object.entries({
  runtimePrototypeSmokeEvidenceTemplateImplemented: true,
...
})) { if (q263[key] !== expected) fail(`manifest Q263 ${key} expected ${expected} got ${q263[key]}`); }
if (!Array.isArray(q263.smokeEvidenceStates) || q263.smokeEvidenceStates.length < 8) fail('Q263 smokeEvidenceStates incomplete');
if (!Array.isArray(q263.requiredSmokeEvidenceFields) || q263.requiredSmokeEvidenceFields.length < 18) fail('Q263 requiredSmokeEvidenceFields incomplete');
if (!Array.isArray(q263.requiredTemplateGates) || q263.requiredTemplateGates.length < 15) fail('Q263 requiredTemplateGates incomplete');
if (!Array.isArray(q263.blockedUntilRealEvidence) || q263.blockedUntilRealEvidence.length < 11) fail('Q263 blockedUntilRealEvidence incomplete');
for (const marker of ['realDeviceRuntimeSmokeJson','realBridgeInvocationSmoke','realLatencyMeasurement','realPeakMemoryMeasurement','realMathLiveReviewImportEvidence','realDirectSolveBlockedEvidence']) if (!q263.blockedUntilRealEvidence.includes(marker)) fail(`Q263 missing blocked evidence ${marker}`);
for (const marker of ['runtime-prototype-smoke-evidence-template-only-no-runtime-no-inference','realDeviceRuntimeSmokeJson','latencyMs','peakMemoryMb','mathLiveReviewOpened','directSolveBlocked','noOcrPassWithoutRealInferenceEvidence']) {
  if (!policy.includes(marker) && !gate.includes(marker) && !audit.includes(marker)) fail(`Q263 marker missing: ${marker}`);
}
for (const marker of ['canAddPaddleRuntimeInQ263 => false','canAddNativeBridgeInQ263 => false','canBindRuntimeMethodChannelInQ263 => false','canRunPpFormulaNetSInference => false','canClaimRuntimeSmokePass => false','canClaimOcrPass => false','canImportCameraOutputWithoutUserApproval => false']) {
  if (!gate.includes(marker)) fail(`gate missing false-claim marker: ${marker}`);
}
for (const marker of ['blockedMissingQ262BridgeGate','blockedMissingSmokeEvidenceState','blockedMissingDeviceAndAbiTemplate','blockedMissingArtifactLoadTemplate','blockedMissingBridgeInvocationTemplate','blockedMissingPerformanceTemplate','blockedUnsafeRuntimeBinding','runtimePrototypeSmokeEvidenceTemplateStaticReady']) {
...
for (const forbidden of ['camera_android_camerax','google_mlkit_text_recognition','paddleocr','paddle_flutter','paddle_lite','onnxruntime','tesseract','pix2text','flutter_downloader','background_downloader','dio']) {
  if (pubspec.toLowerCase().includes(forbidden)) fail(`Q263 must not add dependency: ${forbidden}`);
}
if (!androidManifest.includes('android.permission.CAMERA')) fail('CAMERA permission must remain');
for (const forbidden of ['FormulaRecognition(', 'runPpFormulaNetSInference', 'setWorkspaceLatexWithoutUserApproval', 'startRealNetworkDownloadInQ260', 'addPaddleRuntimeInQ263', 'bindRuntimeMethodChannelInQ263', 'claimRuntimeSmokePassWithoutRealDeviceEvidence']) {
...
]) if (!exists(file)) fail(`Q263 changed-file missing: ${file}`);
if (!changed.includes('gauss_pp_formulanet_s_runtime_prototype_smoke_evidence_template.dart')) fail('changed-files audit missing Q263 Dart file');
if (!readme.includes(phase)) fail('README missing Q263 note');

console.log(JSON.stringify({
  phase,
  status: 'PP_FORMULANET_S_RUNTIME_PROTOTYPE_SMOKE_EVIDENCE_TEMPLATE_Q263_STATIC_READY_BUT_REAL_DEVICE_SMOKE_RUNTIME_INFERENCE_AND_FLUTTER_RETEST_REQUIRED',
  sourcePhase,
  selectedEngineLabel: q263.selectedEngineLabel,
  smokeEvidenceMode: q263.smokeEvidenceMode,
  runtimePrototypeSmokeEvidenceTemplateImplemented: q263.runtimePrototypeSmokeEvidenceTemplateImplemented,
  requiredSmokeEvidenceFields: q263.requiredSmokeEvidenceFields.length,
  requiredTemplateGates: q263.requiredTemplateGates.length,
  blockedUntilRealEvidence: q263.blockedUntilRealEvidence.length,
  runtimeSmokePassClaimed: q263.runtimeSmokePassClaimed,
  ocrPassClaimed: q263.ocrPassClaimed,
  realOcrRuntimeAdded: q263.realOcrRuntimeAdded,
  paddleRuntimeAdded: q263.paddleRuntimeAdded,
  paddleOcrDependencyAdded: q263.paddleOcrDependencyAdded,
  nativeBridgeAdded: q263.nativeBridgeAdded,
  jniBindingAdded: q263.jniBindingAdded,
  methodChannelRuntimeBindingAdded: q263.methodChannelRuntimeBindingAdded,
  modelBinaryBundledInBaseApp: q263.modelBinaryBundledInBaseApp,
  productionInferenceAllowed: q263.productionInferenceAllowed,
