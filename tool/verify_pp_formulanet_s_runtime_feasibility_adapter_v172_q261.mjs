...
const phase = 'V172-Q261-PP-FORMULANET-S-RUNTIME-FEASIBILITY-ADAPTER';
const sourcePhase = 'V172-Q260-PP-FORMULANET-S-REAL-MODEL-DOWNLOAD-INTEGRATION-GATE';
const fail = (message) => { console.error(`PP_FORMULANET_S_RUNTIME_FEASIBILITY_ADAPTER_Q261_BLOCKED: ${message}`); process.exit(1); };
...
for (const path of [
  'lib/features/camera/gauss_pp_formulanet_s_runtime_feasibility_adapter_policy.dart',
  'lib/features/camera/gauss_pp_formulanet_s_runtime_feasibility_adapter.dart',
  ...
  'docs/audit/V172_Q261_PP_FORMULANET_S_RUNTIME_FEASIBILITY_ADAPTER_AUDIT.md',
  'docs/audit/V172_Q261_CHANGED_FILES.md',
]) if (!exists(path)) fail(`required Q261 file missing: ${path}`);

const manifest = JSON.parse(read('assets/mathlive/manifest.json'));
const policy = read('lib/features/camera/gauss_pp_formulanet_s_runtime_feasibility_adapter_policy.dart');
const gate = read('lib/features/camera/gauss_pp_formulanet_s_runtime_feasibility_adapter.dart');
const test = read('test/v172_q261_pp_formulanet_s_runtime_feasibility_adapter_test.dart');
const audit = read('docs/audit/V172_Q261_PP_FORMULANET_S_RUNTIME_FEASIBILITY_ADAPTER_AUDIT.md');
const changed = read('docs/audit/V172_Q261_CHANGED_FILES.md');
...
})) { if (q261[key] !== expected) fail(`manifest Q261 ${key} expected ${expected} got ${q261[key]}`); }
if (!Array.isArray(q261.blockedUntilRealEvidence) || q261.blockedUntilRealEvidence.length < 9) fail('Q261 must list blockedUntilRealEvidence');

for (const marker of [phase, sourcePhase, 'PP-FormulaNet-S', 'runtime-feasibility-adapter-gate-only-no-paddle-runtime-no-inference', 'Paddle Lite / on-device Android shell feasibility', 'officialOnDeviceDeploymentEvidence', 'realAndroidRuntimePrototypeLog', 'directSolveGraphSolutionHistoryBlocked']) {
  if (!policy.includes(marker) && !gate.includes(marker) && !audit.includes(marker)) fail(`Q261 marker missing: ${marker}`);
}
for (const marker of ['paddleRuntimeAdded = false','paddleOcrDependencyAdded = false','nativeBridgeAdded = false','modelBinaryBundledInBaseApp = false','productionInferenceAllowed = false','runtimeFeasibilityPassClaimed = false']) {
  if (!policy.includes(marker)) fail(`policy missing false runtime marker: ${marker}`);
}
for (const marker of ['canAddPaddleRuntimeInQ261 => false','canAddNativeBridgeInQ261 => false','canClaimRuntimeFeasibilityPass => false','canRunPpFormulaNetSInference => false','canClaimOcrPass => false','canImportCameraOutputWithoutUserApproval => false']) {
  if (!gate.includes(marker)) fail(`gate missing false-claim marker: ${marker}`);
}
for (const marker of ['blockedMissingQ260IntegrationGate','blockedMissingRuntimePathEvidence','blockedMissingModelFormatEvidence','blockedMissingAndroidCompatibilityEvidence','blockedMissingLatencyOrMemoryBudget','blockedUnsafeRuntimeOrInferenceBinding','runtimeFeasibilityAdapterStaticReady']) {
...
for (const forbidden of ['camera_android_camerax','google_mlkit_text_recognition','paddleocr','paddle_flutter','paddle_lite','onnxruntime','tesseract','pix2text','flutter_downloader','background_downloader','dio']) {
  if (pubspec.toLowerCase().includes(forbidden)) fail(`Q261 must not add dependency: ${forbidden}`);
}
if (!androidManifest.includes('android.permission.CAMERA')) fail('CAMERA permission must remain');
for (const forbidden of ['FormulaRecognition(', 'runPpFormulaNetSInference', 'setWorkspaceLatexWithoutUserApproval', 'startRealNetworkDownloadInQ260', 'addPaddleRuntimeInQ261', 'NativePaddleBridge']) {
...
]) if (!exists(file)) fail(`Q261 changed-file missing: ${file}`);
if (!changed.includes('gauss_pp_formulanet_s_runtime_feasibility_adapter.dart')) fail('changed-files audit missing Q261 Dart file');
if (!readme.includes(phase)) fail('README missing Q261 note');

console.log(JSON.stringify({
  phase,
  status: 'PP_FORMULANET_S_RUNTIME_FEASIBILITY_ADAPTER_Q261_STATIC_READY_BUT_REAL_RUNTIME_INFERENCE_AND_FLUTTER_RETEST_REQUIRED',
  sourcePhase,
  selectedEngineLabel: q261.selectedEngineLabel,
  feasibilityMode: q261.feasibilityMode,
  officialOnDeviceDeploymentEvidenceRequired: q261.officialOnDeviceDeploymentEvidenceRequired,
  modelFormatEvidenceRequired: q261.modelFormatEvidenceRequired,
  androidRuntimePathEvidenceRequired: q261.androidRuntimePathEvidenceRequired,
  abiCompatibilityEvidenceRequired: q261.abiCompatibilityEvidenceRequired,
  latencyAndPeakMemoryEvidenceRequired: q261.latencyAndPeakMemoryEvidenceRequired,
  runtimeFeasibilityPassClaimed: q261.runtimeFeasibilityPassClaimed,
  realOcrRuntimeAdded: q261.realOcrRuntimeAdded,
  paddleRuntimeAdded: q261.paddleRuntimeAdded,
  paddleOcrDependencyAdded: q261.paddleOcrDependencyAdded,
  nativeBridgeAdded: q261.nativeBridgeAdded,
  modelBinaryBundledInBaseApp: q261.modelBinaryBundledInBaseApp,
  productionInferenceAllowed: q261.productionInferenceAllowed,
