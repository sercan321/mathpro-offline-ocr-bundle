...
const phase = 'V172-Q265-PP-FORMULANET-S-RUNTIME-EVIDENCE-ACCEPTANCE-COURT';
const sourcePhase = 'V172-Q264-PP-FORMULANET-S-RUNTIME-PROTOTYPE-EVIDENCE-INTAKE-GATE';
const fail = (message) => { console.error(`PP_FORMULANET_S_RUNTIME_EVIDENCE_ACCEPTANCE_COURT_Q265_BLOCKED: ${message}`); process.exit(1); };
...
for (const path of [
  'lib/features/camera/gauss_pp_formulanet_s_runtime_evidence_acceptance_court_policy.dart',
  'lib/features/camera/gauss_pp_formulanet_s_runtime_evidence_acceptance_court.dart',
  'test/v172_q265_pp_formulanet_s_runtime_evidence_acceptance_court_test.dart',
  'tool/verify_pp_formulanet_s_runtime_evidence_acceptance_court_v172_q265.mjs',
  'docs/audit/V172_Q265_PP_FORMULANET_S_RUNTIME_EVIDENCE_ACCEPTANCE_COURT_AUDIT.md',
  'docs/audit/V172_Q265_CHANGED_FILES.md',
]) if (!exists(path)) fail(`required Q265 file missing: ${path}`);
...
const policy = read('lib/features/camera/gauss_pp_formulanet_s_runtime_evidence_acceptance_court_policy.dart');
const gate = read('lib/features/camera/gauss_pp_formulanet_s_runtime_evidence_acceptance_court.dart');
const test = read('test/v172_q265_pp_formulanet_s_runtime_evidence_acceptance_court_test.dart');
const audit = read('docs/audit/V172_Q265_PP_FORMULANET_S_RUNTIME_EVIDENCE_ACCEPTANCE_COURT_AUDIT.md');
const changed = read('docs/audit/V172_Q265_CHANGED_FILES.md');
const readme = read('README.md');

if (![phase, 'V172-Q266-PP-FORMULANET-S-RUNTIME-ACTIVATION-DECISION-GATE', 'V172-Q267-PP-FORMULANET-S-RUNTIME-INTEGRATION-PREFLIGHT-PACKAGE', 'V172-Q268-PP-FORMULANET-S-ISOLATED-RUNTIME-IMPLEMENTATION-GATE', 'V172-Q269-PP-FORMULANET-S-RUNTIME-DEPENDENCY-ISOLATION', 'V172-Q270-PP-FORMULANET-S-NATIVE-RUNTIME-BRIDGE-SKELETON', 'V172-Q271-PP-FORMULANET-S-PRIVATE-ARTIFACT-LOAD-PROTOTYPE', 'V172-Q272-PP-FORMULANET-S-FIRST-RUNTIME-SMOKE-PROTOTYPE', 'V172-Q273-PP-FORMULANET-S-IMAGE-TO-LATEX-PROTOTYPE-GATE', 'V172-Q274-OCR-RESULT-MATHLIVE-REAL-REVIEW-BINDING', 'V172-Q275-APPROVED-OCR-WORKSPACE-IMPORT-GATE', 'V172-Q276-CAMERA-OCR-REAL-END-TO-END-DEVICE-COURT', 'V172-Q277-CAMERA-OCR-RUNTIME-PREMIUM-REGRESSION'].includes(manifest.cameraOcrLatestPhase)) fail('manifest cameraOcrLatestPhase must point to this phase or successor through Q277');
if (manifest.ppFormulaNetSRuntimeEvidenceAcceptanceCourtLatestPhase !== phase) fail('manifest missing Q265 latest phase');
const q265 = manifest.v172Q265PpFormulaNetSRuntimeEvidenceAcceptanceCourt;
if (!q265 || q265.phase !== phase) fail('manifest missing Q265 envelope');
if (q265.sourcePhase !== sourcePhase) fail('Q265 sourcePhase mismatch');
if (q265.selectedEngineLabel !== 'PP-FormulaNet-S') fail('Q265 selected engine must be PP-FormulaNet-S');
if (q265.fallbackEngineLabel !== 'PP-FormulaNet_plus-L') fail('Q265 fallback must remain PP-FormulaNet_plus-L');
if (q265.acceptanceCourtMode !== 'runtime-evidence-acceptance-court-only-no-runtime-no-inference') fail('Q265 mode mismatch');
for (const [key, expected] of Object.entries({
  runtimeEvidenceAcceptanceCourtImplemented: true,
  acceptsEvidenceOnlyAfterQ264Validation: true,
  runtimeSmokePassClaimed: false,
  runtimeEvidenceAcceptedWithoutCourt: false,
...
})) { if (q265[key] !== expected) fail(`manifest Q265 ${key} expected ${expected} got ${q265[key]}`); }
if (!Array.isArray(q265.acceptanceStates) || q265.acceptanceStates.length < 12) fail('Q265 acceptanceStates incomplete');
if (!Array.isArray(q265.requiredAcceptanceGates) || q265.requiredAcceptanceGates.length < 20) fail('Q265 requiredAcceptanceGates incomplete');
if (!Array.isArray(q265.rejectionReasons) || q265.rejectionReasons.length < 13) fail('Q265 rejectionReasons incomplete');
if (!Array.isArray(q265.blockedUntilRealEvidence) || q265.blockedUntilRealEvidence.length < 13) fail('Q265 blockedUntilRealEvidence incomplete');
for (const marker of ['acceptedQ264EvidenceJson','realFlutterAnalyzeLog','realFlutterTestLog','realFlutterRunLog','realArtifactSha256Capture','realBridgeInvocationLog','realLatencyMeasurementLog','realPeakMemoryMeasurementLog','realMathLiveReviewOpenedEvidence','realDirectSolveBlockedEvidence']) if (!q265.blockedUntilRealEvidence.includes(marker)) fail(`Q265 missing blocked evidence ${marker}`);
for (const marker of ['runtime-evidence-acceptance-court-only-no-runtime-no-inference','artifactSha256MustBe64Hex','latencyWithinBudget','peakMemoryWithinBudget','mathLiveReviewOpenedTrue','directSolveBlockedTrue','claimRuntimeSmokePassWithoutAcceptedEvidence']) {
  if (!policy.includes(marker) && !gate.includes(marker) && !audit.includes(marker)) fail(`Q265 marker missing: ${marker}`);
}
for (const marker of ['canClaimRuntimeSmokePassInQ265: false','canClaimOcrPassInQ265: false','canAddPaddleRuntimeInQ265: false','canBindRuntimeBridgeInQ265: false','canImportCameraOutputWithoutUserApproval: false','canAcceptEvidenceWithoutCourt: false']) {
  if (!gate.includes(marker)) fail(`gate missing false-claim marker: ${marker}`);
}
for (const marker of ['rejectedMissingQ264IntakeGate','rejectedMissingEvidenceJson','rejectedMalformedSha256','rejectedMissingDeviceAbiRuntimeEvidence','rejectedMissingArtifactEvidence','rejectedMissingBridgeEvidence','rejectedLatencyBudget','rejectedPeakMemoryBudget','rejectedMissingMathLiveReviewEvidence','rejectedMissingDirectSolveBlockedEvidence','rejectedMissingFlutterAnalyzeTestRunLogs','rejectedUnsafeRuntimeBinding','runtimeEvidenceAcceptedForPrototypeReviewOnly']) {
  if (!gate.includes(marker) || !test.includes(marker)) fail(`gate/test missing decision marker: ${marker}`);
}
...
for (const forbidden of ['camera_android_camerax','google_mlkit_text_recognition','paddleocr','paddle_flutter','paddle_lite','onnxruntime','tesseract','pix2text','flutter_downloader','background_downloader','dio']) {
  if (pubspec.toLowerCase().includes(forbidden)) fail(`Q265 must not add dependency: ${forbidden}`);
}
if (!androidManifest.includes('android.permission.CAMERA')) fail('CAMERA permission must remain');
for (const forbidden of ['FormulaRecognition(', 'runPpFormulaNetSInference', 'setWorkspaceLatexWithoutUserApproval', 'startRealNetworkDownloadInQ265', 'addPaddleRuntimeInQ265', 'bindRuntimeMethodChannelInQ265', 'claimRuntimeSmokePassWithoutAcceptedEvidence']) {
...
]) if (!exists(file)) fail(`Q265 changed-file missing: ${file}`);
if (!changed.includes('gauss_pp_formulanet_s_runtime_evidence_acceptance_court.dart')) fail('changed-files audit missing Q265 Dart file');
if (!readme.includes(phase)) fail('README missing Q265 note');

console.log(JSON.stringify({
  phase,
  status: 'PP_FORMULANET_S_RUNTIME_EVIDENCE_ACCEPTANCE_COURT_Q265_STATIC_READY_BUT_REAL_ACCEPTED_EVIDENCE_RUNTIME_INFERENCE_AND_FLUTTER_RETEST_REQUIRED',
  sourcePhase,
  selectedEngineLabel: q265.selectedEngineLabel,
  acceptanceCourtMode: q265.acceptanceCourtMode,
  runtimeEvidenceAcceptanceCourtImplemented: q265.runtimeEvidenceAcceptanceCourtImplemented,
  acceptsEvidenceOnlyAfterQ264Validation: q265.acceptsEvidenceOnlyAfterQ264Validation,
  acceptanceStates: q265.acceptanceStates.length,
  requiredAcceptanceGates: q265.requiredAcceptanceGates.length,
  rejectionReasons: q265.rejectionReasons.length,
  blockedUntilRealEvidence: q265.blockedUntilRealEvidence.length,
  runtimeSmokePassClaimed: q265.runtimeSmokePassClaimed,
  runtimeEvidenceAcceptedWithoutCourt: q265.runtimeEvidenceAcceptedWithoutCourt,
  ocrPassClaimed: q265.ocrPassClaimed,
  realOcrRuntimeAdded: q265.realOcrRuntimeAdded,
  paddleRuntimeAdded: q265.paddleRuntimeAdded,
  paddleOcrDependencyAdded: q265.paddleOcrDependencyAdded,
  nativeBridgeAdded: q265.nativeBridgeAdded,
  jniBindingAdded: q265.jniBindingAdded,
  methodChannelRuntimeBindingAdded: q265.methodChannelRuntimeBindingAdded,
  modelBinaryBundledInBaseApp: q265.modelBinaryBundledInBaseApp,
  productionInferenceAllowed: q265.productionInferenceAllowed,
