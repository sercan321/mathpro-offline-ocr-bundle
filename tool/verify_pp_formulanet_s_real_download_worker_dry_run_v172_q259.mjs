...
const phase = 'V172-Q259-PP-FORMULANET-S-REAL-DOWNLOAD-WORKER-DRY-RUN';
const sourcePhase = 'V172-Q258-PP-FORMULANET-S-PRODUCTION-ARTIFACT-RESOLVER-GATE';
const fail = (message) => { console.error(`PP_FORMULANET_S_REAL_DOWNLOAD_WORKER_DRY_RUN_Q259_BLOCKED: ${message}`); process.exit(1); };
...
]) if (!exists(path)) fail(`required Q259 file missing: ${path}`);

const manifest = JSON.parse(read('assets/mathlive/manifest.json'));
const policy = read('lib/features/camera/gauss_pp_formulanet_s_real_download_worker_dry_run_policy.dart');
const gate = read('lib/features/camera/gauss_pp_formulanet_s_real_download_worker_dry_run.dart');
const test = read('test/v172_q259_pp_formulanet_s_real_download_worker_dry_run_test.dart');
const audit = read('docs/audit/V172_Q259_PP_FORMULANET_S_REAL_DOWNLOAD_WORKER_DRY_RUN_AUDIT.md');
const changed = read('docs/audit/V172_Q259_CHANGED_FILES.md');
...
for (const [key, expected] of Object.entries({
  ...
  modelReadyStateIsDryRunOnly: true,
  checksumMismatchMustBlockReadyState: true,
  atomicMoveRequiresChecksumPass: true,
  ...
  if (q259[key] !== expected) fail(`manifest Q259 ${key} expected ${expected} got ${q259[key]}`);
}
if (!Array.isArray(q259.guardedDryRunStates) || q259.guardedDryRunStates.length < 8) fail('Q259 guarded dry-run states incomplete');
for (const marker of ['dryRunTempFilePrepared','dryRunChecksumMismatch','dryRunAtomicMovePrepared','dryRunFailedRetrySimulation']) if (!q259.guardedDryRunStates.includes(marker)) fail(`Q259 missing dry-run state ${marker}`);
if (!Array.isArray(q259.requiredDryRunSafetyGates) || q259.requiredDryRunSafetyGates.length < 14) fail('Q259 safety gates incomplete');
for (const marker of ['privateTemporaryAppStorageOnly','partialDownloadCleanupDryRun','checksumPassRequiredBeforeAtomicMove','editableMathLiveReviewRequired']) if (!q259.requiredDryRunSafetyGates.includes(marker)) fail(`Q259 missing dry-run safety gate ${marker}`);
if (!Array.isArray(q259.requiredDryRunEvidence) || q259.requiredDryRunEvidence.length < 8) fail('Q259 required dry-run evidence incomplete');
for (const marker of ['checksumMismatchNeverMarksReady','atomicMoveDoesNotRunBeforeChecksumPass','dryRunDoesNotClaimModelDownloadPass','dryRunDoesNotClaimOcrPass']) if (!q259.requiredDryRunEvidence.includes(marker)) fail(`Q259 missing dry-run evidence ${marker}`);
if (!Array.isArray(q259.forbiddenActions) || q259.forbiddenActions.length < 16) fail('Q259 forbidden actions incomplete');
for (const marker of ['startProductionNetworkDownloadInQ259','runPpFormulaNetSInference','claimModelDownloadPass','setWorkspaceLatexWithoutUserApproval']) if (!q259.forbiddenActions.includes(marker)) fail(`Q259 forbidden action missing ${marker}`);

for (const marker of [phase, 'real-download-worker-dry-run-no-production-network', 'privateTemporaryAppStorageOnly', 'partialDownloadCleanupDryRun', 'checksumPassRequiredBeforeAtomicMove', 'editable MathLive review']) {
  if (!policy.includes(marker) && !gate.includes(marker) && !audit.includes(marker) && !readme.includes(marker)) fail(`missing Q259 marker: ${marker}`);
}
for (const marker of ['canStartProductionNetworkDownload => false','canBindProductionModelUrl => false','canClaimModelDownloadPass => false','canClaimOcrPass => false','canRunPpFormulaNetSInference => false','canMarkReadyAfterChecksumMismatch => false','canAtomicMoveBeforeChecksumPass => false','canImportCameraOutputWithoutUserApproval => false']) if (!gate.includes(marker)) fail(`gate missing false-claim marker: ${marker}`);
for (const marker of ['blockedMissingQ258ResolverGate','blockedMissingChecksumSimulation','blockedMissingAtomicMoveSafety','realDownloadWorkerDryRunStaticReady']) if (!gate.includes(marker) || !test.includes(marker)) fail(`gate/test missing decision marker: ${marker}`);
...
for (const forbidden of ['camera_android_camerax','google_mlkit_text_recognition','paddleocr','paddle_flutter','onnxruntime','tesseract','pix2text','flutter_downloader','background_downloader','dio']) {
  if (pubspec.toLowerCase().includes(forbidden)) fail(`Q259 must not add dependency: ${forbidden}`);
}
if (!androidManifest.includes('android.permission.CAMERA')) fail('CAMERA permission must remain');
if (androidManifest.includes('READ_MEDIA_IMAGES')) fail('Q259 must not add media permissions');
for (const forbidden of ['FormulaRecognition(', 'runPpFormulaNetSInference', 'setWorkspaceLatexWithoutUserApproval', 'startProductionNetworkDownloadInQ259', 'downloadPpFormulaNetSModel', 'bindProductionModelUrlInQ259']) {
...
]) if (!exists(file)) fail(`Q259 changed-file missing: ${file}`);
if (!changed.includes('gauss_pp_formulanet_s_real_download_worker_dry_run.dart')) fail('changed-files audit missing Q259 Dart file');

console.log(JSON.stringify({
  phase,
  status: 'PP_FORMULANET_S_REAL_DOWNLOAD_WORKER_DRY_RUN_Q259_STATIC_READY_BUT_REAL_DOWNLOADER_MODEL_RUNTIME_AND_FLUTTER_RETEST_REQUIRED',
  sourcePhase,
  selectedEngineLabel: q259.selectedEngineLabel,
  dryRunMode: q259.dryRunMode,
  guardedDryRunStates: q259.guardedDryRunStates.length,
  requiredDryRunSafetyGates: q259.requiredDryRunSafetyGates.length,
  requiredDryRunEvidence: q259.requiredDryRunEvidence.length,
  productionDownloadEnabled: q259.productionDownloadEnabled,
  productionModelUrlBound: q259.productionModelUrlBound,
  realNetworkDownloadWorkerImplemented: q259.realNetworkDownloadWorkerImplemented,
  realOcrRuntimeAdded: q259.realOcrRuntimeAdded,
  paddleRuntimeAdded: q259.paddleRuntimeAdded,
  modelBinaryBundledInBaseApp: q259.modelBinaryBundledInBaseApp,
  productionInferenceAllowed: q259.productionInferenceAllowed,
