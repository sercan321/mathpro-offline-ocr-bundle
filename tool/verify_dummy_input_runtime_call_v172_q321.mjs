...
const phase = 'V172-Q321-DUMMY-INPUT-RUNTIME-CALL';
const sourcePhase = 'V172-Q320-RUNTIME-STARTUP-SMOKE-ON-DEVICE';
...
if (![phase, q322SuccessorPhase, q323SuccessorPhase, q324SuccessorPhase, q325SuccessorPhase, q326SuccessorPhase, q327SuccessorPhase, q328SuccessorPhase, q329SuccessorPhase, q330SuccessorPhase, q331SuccessorPhase, q332SuccessorPhase, q333SuccessorPhase, q334SuccessorPhase, q335SuccessorPhase, q336SuccessorPhase, q337SuccessorPhase, q338SuccessorPhase, q339SuccessorPhase, q340SuccessorPhase, q341SuccessorPhase, q342SuccessorPhase].includes(manifest.cameraOcrLatestPhase)) fail(`cameraOcrLatestPhase must be Q321 or Q322 successor, got ${manifest.cameraOcrLatestPhase}`);
if (![phase, q322SuccessorPhase, q323SuccessorPhase, q324SuccessorPhase, q325SuccessorPhase, q326SuccessorPhase, q327SuccessorPhase, q328SuccessorPhase, q329SuccessorPhase, q330SuccessorPhase, q331SuccessorPhase, q332SuccessorPhase, q333SuccessorPhase, q334SuccessorPhase, q335SuccessorPhase, q336SuccessorPhase, q337SuccessorPhase, q338SuccessorPhase, q339SuccessorPhase, q340SuccessorPhase, q341SuccessorPhase, q342SuccessorPhase].includes(manifest.activeProductDevelopmentLatestPhase)) fail(`activeProductDevelopmentLatestPhase must be Q321 or Q322 successor, got ${manifest.activeProductDevelopmentLatestPhase}`);
...
if (manifest.dummyInputRuntimeCallLatestPhase !== phase) fail('latest Q321 marker missing');
if (manifest.runtimeStartupSmokeOnDeviceLatestPhase !== sourcePhase) fail('Q320 source latest marker missing');
if (!manifest.v172Q320RuntimeStartupSmokeOnDevice) fail('Q320 source envelope missing');
const q321 = manifest.v172Q321DummyInputRuntimeCall;
if (!q321) fail('Q321 manifest envelope missing');
if (q321.phase !== phase) fail('Q321 manifest phase mismatch');
if (q321.sourcePhase !== sourcePhase) fail('Q321 source phase mismatch');
if (q321.nativeChannelName !== 'mathpro/ocr_runtime_bridge') fail('Q321 native channel mismatch');
if (q321.dummyRuntimeCallMethod !== 'dummyRuntimeCall') fail('Q321 method mismatch');
if (q321.dummyRuntimeCallStatus !== 'DUMMY_RUNTIME_CALL_BLOCKED_NO_RUNTIME_DEPENDENCY_DEFAULT_OFF') fail('Q321 status mismatch');
if (q321.expectedInferencePdiparamsSha256 !== expectedSha256) fail('Q321 expected SHA mismatch');
if (q321.expectedInferencePdiparamsSizeBytes !== 231675001) fail('Q321 artifact size evidence mismatch');
for (const key of [
  ...
  'dummyRuntimeCallMethodAdded',
  'mainActivityChanged',
  'protectedMainActivityChangeDeclared',
  'methodChannelBindingExtended'
]) {
  if (q321[key] !== true) fail(`Q321 ${key} must be true`);
}
for (const key of [
  'q315RuntimeAccepted',
  ...
  if (q321[key] !== false) fail(`Q321 ${key} must remain false`);
}
for (const field of ['phase','sourcePhase','method','channel','status','dummyPayloadLabel','dummyPayloadSha256','artifactPath','artifactExists','artifactSizeBytes','expectedArtifactSha256','actualArtifactSha256','sha256MatchesExpected','verifiedArtifactLoadSmokePassed','dummyRuntimeCallBlockedReason','runtimeDependencyAdded','runtimeStartupAttempted','runtimeStartupExecuted','dummyRuntimeCallAttempted','dummyRuntimeCallExecuted','dummyRuntimeCallPassed','realImageToLatexInferenceExecuted','editableMathLiveReviewOpened','workspaceImportExecuted','ocrPassClaimed']) {
  if (!q321.dummyRuntimeCallEnvelopeFields.includes(field)) fail(`Q321 envelope field missing: ${field}`);
}
for (const item of ['no-paddle-onnx-tflite-runtime-dependency-in-q321','no-jni-native-model-loader-in-q321','no-dummy-runtime-call-execution-without-selected-runtime-dependency-in-q321','no-image-to-latex-in-q321','no-ocr-pass-or-android-real-device-pass-without-real-run-log']) {
  if (!q321.q321HardBlocks.includes(item)) fail(`Q321 hard block missing: ${item}`);
}
for (const item of ['q321-flutter-run-device-log-shows-dummyRuntimeCall-method-response','q321-response-keeps-dummyRuntimeCallExecuted-false-with-no-runtime-dependency','runtime-dependency-selection-approved-before-first-image-to-latex']) {
  if (!q321.q322EntryCriteria.includes(item)) fail(`Q322 entry criterion missing: ${item}`);
}
...
for (const file of [
  ...
  if (!fs.existsSync(file)) fail(`Q321 file missing: ${file}`);
}
const policy = read('lib/features/camera/gauss_dummy_input_runtime_call_q321_policy.dart');
for (const marker of [phase, sourcePhase, 'dummyRuntimeCall', 'DUMMY_RUNTIME_CALL_BLOCKED_NO_RUNTIME_DEPENDENCY_DEFAULT_OFF', 'gauss.ppFormulaNetS.dummyRuntimeCall.defaultOff', expectedSha256]) {
  if (!policy.includes(marker)) fail(`Q321 policy marker missing: ${marker}`);
}
const impl = read('lib/features/camera/gauss_dummy_input_runtime_call_q321.dart');
if (!impl.includes('return const GaussDummyInputRuntimeCallQ321Result(')) fail('Q321 evaluate must return const result envelope');
...
for (const marker of [
  ...
  if (!mainActivity.includes(marker)) fail(`Q321 MainActivity marker missing: ${marker}`);
}
for (const forbidden of ['PaddleOCR','PaddleLite','loadModel(','runInference(','imageToLatex(','JNIEnv','OkHttpClient','DownloadManager']) {
  if (mainActivity.includes(forbidden)) fail(`forbidden runtime/download marker in Q321 MainActivity: ${forbidden}`);
}

const changed = read('docs/audit/V172_Q321_CHANGED_FILES.md');
if (!changed.includes('Protected Runtime File Intentionally Modified')) fail('Q321 changed files must declare intentional protected MainActivity change');
if (!changed.includes('android/app/src/main/kotlin/com/mathpro/flutter/MainActivity.kt')) fail('Q321 changed files must list MainActivity');
if (!changed.includes('default-off dummy runtime call envelope method only')) fail('Q321 changed files must explain MainActivity reason');
const audit = read('docs/audit/V172_Q321_DUMMY_INPUT_RUNTIME_CALL_AUDIT.md');
if (!audit.includes('Runtime accepted: `false`')) fail('Q321 audit must keep runtime acceptance false');
if (!audit.includes('DUMMY_RUNTIME_CALL_BLOCKED_NO_RUNTIME_DEPENDENCY_DEFAULT_OFF')) fail('Q321 audit must disclose blocked dummy call status');
const readme = read('README.md');
if (!readme.includes('V172-Q321')) fail('README must mention Q321');
if (!readme.includes('DUMMY_RUNTIME_CALL_BLOCKED_NO_RUNTIME_DEPENDENCY_DEFAULT_OFF')) fail('README must disclose Q321 smoke status');
...
for (const forbidden of ['paddleocr','paddle_lite','paddle-lite','onnxruntime','tflite_flutter','workmanager','background_downloader','dio','http:','flutter_downloader']) {
  if (pubspec.includes(forbidden)) fail(`forbidden runtime/download dependency added in Q321: ${forbidden}`);
}
...
for (const forbidden of ['READ_EXTERNAL_STORAGE','MANAGE_EXTERNAL_STORAGE','FOREGROUND_SERVICE_DATA_SYNC']) {
  if (androidManifest.includes(forbidden)) fail(`forbidden Android permission added in Q321: ${forbidden}`);
}
...
console.log(JSON.stringify({
  phase,
  status: manifest.cameraOcrLatestPhase === q322SuccessorPhase ? 'DUMMY_RUNTIME_CALL_Q322_SUCCESSOR_AWARE' : 'DUMMY_RUNTIME_CALL_BOUND_DEFAULT_OFF_NO_RUNTIME_DEPENDENCY',
  sourcePhase,
  nativeChannelName: q321.nativeChannelName,
  dummyRuntimeCallMethod: q321.dummyRuntimeCallMethod,
  runtimeDependencyAdded: q321.runtimeDependencyAdded,
  runtimeStartupAttempted: q321.runtimeStartupAttempted,
  runtimeStartupExecuted: q321.runtimeStartupExecuted,
  dummyRuntimeCallAttempted: q321.dummyRuntimeCallAttempted,
  dummyRuntimeCallExecuted: q321.dummyRuntimeCallExecuted,
  dummyRuntimeCallPassed: q321.dummyRuntimeCallPassed,
  modelArtifactAcceptedForRuntime: q321.modelArtifactAcceptedForRuntime,
  realImageToLatexInferenceExecuted: q321.realImageToLatexInferenceExecuted,
  ocrPassClaimed: q321.ocrPassClaimed,
  androidRealDevicePassClaimed: q321.androidRealDevicePassClaimed
}, null, 2));
