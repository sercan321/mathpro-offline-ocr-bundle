...
const phase = 'V172-Q319-VERIFIED-ARTIFACT-LOAD-SMOKE-ON-DEVICE';
const sourcePhase = 'V172-Q318-PRIVATE-MODEL-DOWNLOAD-STORAGE-REAL-IMPLEMENTATION';
...
if (![phase, q320SuccessorPhase, q321SuccessorPhase, q322SuccessorPhase, q323SuccessorPhase, q324SuccessorPhase, q325SuccessorPhase, q326SuccessorPhase, q327SuccessorPhase, q328SuccessorPhase, q329SuccessorPhase, q330SuccessorPhase, q331SuccessorPhase, q332SuccessorPhase, q333SuccessorPhase, q334SuccessorPhase, q335SuccessorPhase, q336SuccessorPhase, q337SuccessorPhase, q338SuccessorPhase, q339SuccessorPhase, q340SuccessorPhase, q341SuccessorPhase, q342SuccessorPhase].includes(manifest.cameraOcrLatestPhase)) fail(`cameraOcrLatestPhase must be Q319 or Q320 successor, got ${manifest.cameraOcrLatestPhase}`);
if (![phase, q320SuccessorPhase, q321SuccessorPhase, q322SuccessorPhase, q323SuccessorPhase, q324SuccessorPhase, q325SuccessorPhase, q326SuccessorPhase, q327SuccessorPhase, q328SuccessorPhase, q329SuccessorPhase, q330SuccessorPhase, q331SuccessorPhase, q332SuccessorPhase, q333SuccessorPhase, q334SuccessorPhase, q335SuccessorPhase, q336SuccessorPhase, q337SuccessorPhase, q338SuccessorPhase, q339SuccessorPhase, q340SuccessorPhase, q341SuccessorPhase, q342SuccessorPhase].includes(manifest.activeProductDevelopmentLatestPhase)) fail(`activeProductDevelopmentLatestPhase must be Q319 or Q320 successor, got ${manifest.activeProductDevelopmentLatestPhase}`);
if (manifest.verifiedArtifactLoadSmokeOnDeviceLatestPhase !== phase) fail('latest Q319 marker missing');
if (manifest.privateModelDownloadStorageRealImplementationLatestPhase !== sourcePhase) fail('Q318 source latest marker missing');
if (!manifest.v172Q318PrivateModelDownloadStorageRealImplementation) fail('Q318 envelope missing');
...
const q319 = manifest.v172Q319VerifiedArtifactLoadSmokeOnDevice;
if (!q319) fail('Q319 manifest envelope missing');
if (q319.phase !== phase) fail('Q319 manifest phase mismatch');
if (q319.sourcePhase !== sourcePhase) fail('Q319 source phase mismatch');
if (q319.nativeChannelName !== 'mathpro/ocr_runtime_bridge') fail('Q319 native channel mismatch');
if (q319.verifiedArtifactLoadSmokeMethod !== 'verifiedArtifactLoadSmoke') fail('Q319 method mismatch');
if (q319.verifiedArtifactLoadSmokeStatus !== 'VERIFIED_ARTIFACT_LOAD_SMOKE_AVAILABLE_DEFAULT_OFF') fail('Q319 status mismatch');
if (q319.expectedInferencePdiparamsSha256 !== expectedSha256) fail('Q319 expected SHA mismatch');
if (q319.expectedInferencePdiparamsSizeBytes !== 231675001) fail('Q319 artifact size evidence mismatch');
for (const key of [
  'q315HashEvidenceProvidedToAssistant',
  'q315Sha256MatchesExpected',
  'q315ArtifactStoredOutsideProjectRoot',
  'q318PrivateStorageHealthcheckAdded',
  'verifiedArtifactLoadSmokeMethodAdded',
  'mainActivityChanged',
  'protectedMainActivityChangeDeclared',
  'methodChannelBindingExtended'
]) {
  if (q319[key] !== true) fail(`Q319 ${key} must be true`);
}
for (const key of [
  'q315RuntimeAccepted',
  ...
  if (q319[key] !== false) fail(`Q319 ${key} must remain false`);
}
for (const field of ['phase','sourcePhase','method','channel','status','artifactPath','artifactExists','artifactSizeBytes','expectedArtifactSha256','actualArtifactSha256','sha256MatchesExpected','artifactOpenedForRead','artifactReadSmokeBytes','artifactSmokeError','verifiedArtifactLoadSmokePassed','modelArtifactAcceptedForRuntime','runtimeDependencyAdded','runtimeStartupExecuted','modelLoaded','imageToLatexInferenceExecuted','editableMathLiveReviewOpened','workspaceImportExecuted','ocrPassClaimed']) {
  if (!q319.verifiedArtifactLoadSmokeEnvelopeFields.includes(field)) fail(`Q319 envelope field missing: ${field}`);
}
for (const item of ['no-model-binary-committed-to-project-zip-in-q319','no-runtime-dependency-or-jni-handler-in-q319','no-native-model-loader-or-runtime-acceptance-in-q319','no-runtime-startup-dummy-call-or-image-to-latex-in-q319','no-ocr-pass-or-android-real-device-pass-without-real-run-log']) {
  if (!q319.q319HardBlocks.includes(item)) fail(`Q319 hard block missing: ${item}`);
}
for (const item of ['q319-flutter-run-device-log-shows-verified-artifact-load-smoke','verified-artifact-load-smoke-passed-true-on-device-before-runtime-startup','feature-flag-default-off-remains-enforced']) {
  if (!q319.q320EntryCriteria.includes(item)) fail(`Q320 entry criterion missing: ${item}`);
}

for (const file of [
  'lib/features/camera/gauss_verified_artifact_load_smoke_on_device_q319_policy.dart',
  'lib/features/camera/gauss_verified_artifact_load_smoke_on_device_q319.dart',
  ...
  'docs/audit/V172_Q319_VERIFIED_ARTIFACT_LOAD_SMOKE_ON_DEVICE_AUDIT.md',
  'docs/audit/V172_Q319_CHANGED_FILES.md'
]) {
  if (!fs.existsSync(file)) fail(`Q319 file missing: ${file}`);
}
const policy = read('lib/features/camera/gauss_verified_artifact_load_smoke_on_device_q319_policy.dart');
for (const marker of [phase, sourcePhase, 'verifiedArtifactLoadSmoke', 'VERIFIED_ARTIFACT_LOAD_SMOKE_AVAILABLE_DEFAULT_OFF', 'gauss.ppFormulaNetS.verifiedArtifactLoadSmoke.defaultOff', expectedSha256]) {
  if (!policy.includes(marker)) fail(`Q319 policy marker missing: ${marker}`);
}
const impl = read('lib/features/camera/gauss_verified_artifact_load_smoke_on_device_q319.dart');
if (!impl.includes('return const GaussVerifiedArtifactLoadSmokeOnDeviceQ319Result(')) fail('Q319 evaluate must return const result envelope');
...
for (const marker of [
  ...
  if (!mainActivity.includes(marker)) fail(`Q319 MainActivity marker missing: ${marker}`);
}
for (const forbidden of ['PaddleOCR','PaddleLite','loadModel(','runInference(','imageToLatex(','JNIEnv','OkHttpClient','DownloadManager']) {
  if (mainActivity.includes(forbidden)) fail(`forbidden runtime/download marker in Q319 MainActivity: ${forbidden}`);
}

const changed = read('docs/audit/V172_Q319_CHANGED_FILES.md');
if (!changed.includes('Protected Runtime File Intentionally Modified')) fail('Q319 changed files must declare intentional protected MainActivity change');
if (!changed.includes('android/app/src/main/kotlin/com/mathpro/flutter/MainActivity.kt')) fail('Q319 changed files must list MainActivity');
if (!changed.includes('verified artifact file open/read/SHA smoke method only')) fail('Q319 changed files must explain MainActivity reason');
const audit = read('docs/audit/V172_Q319_VERIFIED_ARTIFACT_LOAD_SMOKE_ON_DEVICE_AUDIT.md');
if (!audit.includes('SHA match: `true`')) fail('Q319 audit must record Q315 SHA match');
if (!audit.includes('Runtime accepted: `false`')) fail('Q319 audit must keep runtime acceptance false');
const readme = read('README.md');
if (!readme.includes('V172-Q319')) fail('README must mention Q319');
if (!readme.includes('VERIFIED_ARTIFACT_LOAD_SMOKE_AVAILABLE_DEFAULT_OFF')) fail('README must disclose Q319 smoke status');
...
for (const forbidden of ['paddleocr','paddle_lite','paddle-lite','onnxruntime','tflite_flutter','workmanager','background_downloader','dio','http:','flutter_downloader']) {
  if (pubspec.includes(forbidden)) fail(`forbidden runtime/download dependency added in Q319: ${forbidden}`);
}
...
for (const forbidden of ['READ_EXTERNAL_STORAGE','MANAGE_EXTERNAL_STORAGE','FOREGROUND_SERVICE_DATA_SYNC']) {
  if (androidManifest.includes(forbidden)) fail(`forbidden Android permission added in Q319: ${forbidden}`);
}
...
console.log(JSON.stringify({
  phase,
  status: 'VERIFIED_ARTIFACT_LOAD_SMOKE_BOUND_DEFAULT_OFF_NO_RUNTIME',
  sourcePhase,
  nativeChannelName: q319.nativeChannelName,
  verifiedArtifactLoadSmokeMethod: q319.verifiedArtifactLoadSmokeMethod,
  q315Sha256MatchesExpected: q319.q315Sha256MatchesExpected,
  q318PrivateStorageHealthcheckAdded: q319.q318PrivateStorageHealthcheckAdded,
  runtimeDependencyAdded: q319.runtimeDependencyAdded,
  modelArtifactAcceptedForRuntime: q319.modelArtifactAcceptedForRuntime,
  runtimeStartupExecuted: q319.runtimeStartupExecuted,
  realImageToLatexInferenceExecuted: q319.realImageToLatexInferenceExecuted,
  ocrPassClaimed: q319.ocrPassClaimed,
  androidRealDevicePassClaimed: q319.androidRealDevicePassClaimed
}, null, 2));
