...
const phase = 'V172-Q255-PP-FORMULANET-S-MODEL-DOWNLOAD-WORKER-GUARD';
const sourcePhase = 'V172-Q254-PP-FORMULANET-S-RUNTIME-SOURCE-EVIDENCE-GATE';
function read(path) { return fs.readFileSync(`${root}/${path}`, 'utf8'); }
function exists(path) { return fs.existsSync(`${root}/${path}`); }
function fail(message) { console.error(`PP_FORMULANET_S_MODEL_DOWNLOAD_WORKER_GUARD_Q255_BLOCKED: ${message}`); process.exit(1); }
function sha(path) { return crypto.createHash('sha256').update(fs.readFileSync(`${root}/${path}`)).digest('hex'); }

const policyPath = 'lib/features/camera/gauss_pp_formulanet_s_model_download_worker_guard_policy.dart';
const guardPath = 'lib/features/camera/gauss_pp_formulanet_s_model_download_worker_guard.dart';
const testPath = 'test/v172_q255_pp_formulanet_s_model_download_worker_guard_test.dart';
const auditPath = 'docs/audit/V172_Q255_PP_FORMULANET_S_MODEL_DOWNLOAD_WORKER_GUARD_AUDIT.md';
const changedPath = 'docs/audit/V172_Q255_CHANGED_FILES.md';
for (const path of [policyPath, guardPath, testPath, auditPath, changedPath]) if (!exists(path)) fail(`missing ${path}`);
...
for (const marker of [
  ...
  'privateAppStorageRequired = true',
  'temporaryDownloadPathRequired = true',
  'atomicMoveAfterVerifyRequired = true',
  'corruptDownloadCleanupRequired = true',
  'partialDownloadCleanupRequired = true',
  'offlineAfterInstallRequired = true',
  'userConsentOrWifiGateRequired = true',
]) if (!policy.includes(marker)) fail(`Q255 policy missing marker: ${marker}`);
...
for (const marker of [
  ...
  'requiresTemporaryPrivateFile = true',
  'requiresAtomicMoveAfterVerify = true',
  'allowsProductionModelUrlInQ255 = false',
  'implementsRealNetworkDownloadWorkerInQ255 = false',
  'allowsProductionInferenceInQ255 = false',
  'Partial downloads and corrupt artifacts must be deleted',
]) if (!guard.includes(marker)) fail(`Q255 guard missing marker: ${marker}`);

for (const marker of [
  phase,
  'records PP-FormulaNet-S model download worker guard without real downloader',
  'passes only when checksum, private storage, cleanup, offline, and review guards are intact',
  'blocks missing source, checksum, storage, cleanup, network, unsafe binding, and review regressions',
  'keeps app dependencies, production runtime, and base APK clean',
  'camera: 0.10.6',
...
for (const [key, expected] of [
  ...
  ['atomicMoveAfterVerifyRequired', true],
  ['partialDownloadCleanupRequired', true],
  ['corruptArtifactCleanupRequired', true],
  ['offlineAfterInstallRequired', true],
...
]) if (q255[key] !== expected) fail(`manifest Q255 ${key} mismatch`);
if (!Array.isArray(q255.guardedInstallStates) || q255.guardedInstallStates.length !== 8) fail('Q255 must list 8 guarded install states');
if (!Array.isArray(q255.requiredWorkerSafetyGates) || q255.requiredWorkerSafetyGates.length < 14) fail('Q255 must list required worker safety gates');
if (!Array.isArray(q255.forbiddenActions) || q255.forbiddenActions.length < 15) fail('Q255 must list forbidden actions');
...
for (const forbidden of ['camera_android_camerax','google_mlkit_text_recognition','paddleocr','paddle_flutter','onnxruntime','tesseract','pix2text','flutter_downloader','background_downloader','dio']) {
  if (pubspec.toLowerCase().includes(forbidden)) fail(`Q255 must not add dependency: ${forbidden}`);
}
if (!androidManifest.includes('android.permission.CAMERA')) fail('CAMERA permission must remain');
if (androidManifest.includes('READ_MEDIA_IMAGES')) fail('Q255 must not add media permissions');
for (const forbidden of ['FormulaRecognition(', 'runPpFormulaNetSInference', 'setWorkspaceLatexWithoutUserApproval', 'downloadPpFormulaNetSModel']) {
...
for (const marker of [phase, 'Q255', 'PP-FormulaNet-S', 'download worker guard', 'expected SHA256', 'actual SHA256', 'private temporary', 'atomic move', 'partial', 'corrupt', 'offline-after-install', 'no production model URL', 'no model binary', 'no Paddle runtime', 'editable MathLive review', 'user approval', 'Q254']) {
  if (!readme.includes(marker)) fail(`README missing Q255 marker: ${marker}`);
  if (!audit.includes(marker)) fail(`audit missing Q255 marker: ${marker}`);
}
for (const file of [
  policyPath,
  guardPath,
...
console.log(JSON.stringify({
  phase,
  status: 'PP_FORMULANET_S_MODEL_DOWNLOAD_WORKER_GUARD_Q255_STATIC_READY_BUT_REAL_DOWNLOADER_MODEL_RUNTIME_AND_FLUTTER_RETEST_REQUIRED',
  sourcePhase,
  selectedEngineLabel: q255.selectedEngineLabel,
  guardMode: q255.guardMode,
  guardedInstallStates: q255.guardedInstallStates.length,
  requiredWorkerSafetyGates: q255.requiredWorkerSafetyGates.length,
  realOcrRuntimeAdded: q255.realOcrRuntimeAdded,
  paddleRuntimeAdded: q255.paddleRuntimeAdded,
  modelBinaryBundledInBaseApp: q255.modelBinaryBundledInBaseApp,
  productionModelUrlBound: q255.productionModelUrlBound,
  realNetworkDownloadWorkerImplemented: q255.realNetworkDownloadWorkerImplemented,
  productionInferenceAllowed: q255.productionInferenceAllowed,
