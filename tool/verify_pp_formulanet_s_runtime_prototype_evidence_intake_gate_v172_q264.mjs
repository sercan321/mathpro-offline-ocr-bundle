...
const phase = 'V172-Q264-PP-FORMULANET-S-RUNTIME-PROTOTYPE-EVIDENCE-INTAKE-GATE';
const sourcePhase = 'V172-Q263-PP-FORMULANET-S-RUNTIME-PROTOTYPE-SMOKE-EVIDENCE-TEMPLATE';
const fail = (message) => { console.error(`PP_FORMULANET_S_RUNTIME_PROTOTYPE_EVIDENCE_INTAKE_GATE_Q264_BLOCKED: ${message}`); process.exit(1); };
...
for (const path of [
  ...
  'docs/audit/V172_Q264_PP_FORMULANET_S_RUNTIME_PROTOTYPE_EVIDENCE_INTAKE_GATE_AUDIT.md',
  'docs/audit/V172_Q264_CHANGED_FILES.md',
]) if (!exists(path)) fail(`required Q264 file missing: ${path}`);
...
const policy = read('lib/features/camera/gauss_pp_formulanet_s_runtime_prototype_evidence_intake_gate_policy.dart');
const gate = read('lib/features/camera/gauss_pp_formulanet_s_runtime_prototype_evidence_intake_gate.dart');
const test = read('test/v172_q264_pp_formulanet_s_runtime_prototype_evidence_intake_gate_test.dart');
const audit = read('docs/audit/V172_Q264_PP_FORMULANET_S_RUNTIME_PROTOTYPE_EVIDENCE_INTAKE_GATE_AUDIT.md');
const changed = read('docs/audit/V172_Q264_CHANGED_FILES.md');
const readme = read('README.md');

if (![phase, 'V172-Q265-PP-FORMULANET-S-RUNTIME-EVIDENCE-ACCEPTANCE-COURT', 'V172-Q266-PP-FORMULANET-S-RUNTIME-ACTIVATION-DECISION-GATE', 'V172-Q267-PP-FORMULANET-S-RUNTIME-INTEGRATION-PREFLIGHT-PACKAGE', 'V172-Q268-PP-FORMULANET-S-ISOLATED-RUNTIME-IMPLEMENTATION-GATE', 'V172-Q269-PP-FORMULANET-S-RUNTIME-DEPENDENCY-ISOLATION', 'V172-Q270-PP-FORMULANET-S-NATIVE-RUNTIME-BRIDGE-SKELETON', 'V172-Q271-PP-FORMULANET-S-PRIVATE-ARTIFACT-LOAD-PROTOTYPE', 'V172-Q272-PP-FORMULANET-S-FIRST-RUNTIME-SMOKE-PROTOTYPE', 'V172-Q273-PP-FORMULANET-S-IMAGE-TO-LATEX-PROTOTYPE-GATE', 'V172-Q274-OCR-RESULT-MATHLIVE-REAL-REVIEW-BINDING', 'V172-Q275-APPROVED-OCR-WORKSPACE-IMPORT-GATE', 'V172-Q276-CAMERA-OCR-REAL-END-TO-END-DEVICE-COURT', 'V172-Q277-CAMERA-OCR-RUNTIME-PREMIUM-REGRESSION'].includes(manifest.cameraOcrLatestPhase)) fail('manifest cameraOcrLatestPhase must point to this phase or successor through Q277');
if (manifest.ppFormulaNetSRuntimePrototypeEvidenceIntakeGateLatestPhase !== phase) fail('manifest missing Q264 latest phase');
const q264 = manifest.v172Q264PpFormulaNetSRuntimePrototypeEvidenceIntakeGate;
if (!q264 || q264.phase !== phase) fail('manifest missing Q264 envelope');
if (q264.sourcePhase !== sourcePhase) fail('Q264 sourcePhase mismatch');
if (q264.selectedEngineLabel !== 'PP-FormulaNet-S') fail('Q264 selected engine must be PP-FormulaNet-S');
if (q264.fallbackEngineLabel !== 'PP-FormulaNet_plus-L') fail('Q264 fallback must remain PP-FormulaNet_plus-L');
if (q264.evidenceIntakeMode !== 'runtime-prototype-evidence-intake-gate-only-no-runtime-no-inference') fail('Q264 evidence intake mode mismatch');
for (const [key, expected] of Object.entries({
  runtimePrototypeEvidenceIntakeGateImplemented: true,
...
})) { if (q264[key] !== expected) fail(`manifest Q264 ${key} expected ${expected} got ${q264[key]}`); }
if (!Array.isArray(q264.intakeStates) || q264.intakeStates.length < 8) fail('Q264 intakeStates incomplete');
if (!Array.isArray(q264.requiredEvidenceJsonFields) || q264.requiredEvidenceJsonFields.length < 18) fail('Q264 requiredEvidenceJsonFields incomplete');
if (!Array.isArray(q264.requiredIntakeValidationGates) || q264.requiredIntakeValidationGates.length < 19) fail('Q264 requiredIntakeValidationGates incomplete');
if (!Array.isArray(q264.blockedUntilRealEvidence) || q264.blockedUntilRealEvidence.length < 13) fail('Q264 blockedUntilRealEvidence incomplete');
for (const marker of ['realDeviceSmokeEvidenceJsonFile','realFlutterAnalyzeLog','realFlutterTestLog','realFlutterRunLog','realArtifactSha256Capture','realBridgeInvocationLog','realLatencyMeasurementLog','realPeakMemoryMeasurementLog','realMathLiveReviewOpenedEvidence','realDirectSolveBlockedEvidence']) if (!q264.blockedUntilRealEvidence.includes(marker)) fail(`Q264 missing blocked evidence ${marker}`);
for (const marker of ['runtime-prototype-evidence-intake-gate-only-no-runtime-no-inference','artifactSha256MustBe64Hex','mathLiveReviewOpenedMustBeTrue','directSolveBlockedMustBeTrue','acceptRuntimeEvidenceWithoutSchemaValidation','realDeviceSmokeEvidenceJsonFile']) {
  if (!policy.includes(marker) && !gate.includes(marker) && !audit.includes(marker)) fail(`Q264 marker missing: ${marker}`);
}
for (const marker of ['canAcceptEvidenceWithoutValidation => false','canClaimRuntimeSmokePassInQ264 => false','canClaimOcrPassInQ264 => false','canAddPaddleRuntimeInQ264 => false','canBindRuntimeBridgeInQ264 => false','canImportCameraOutputWithoutUserApproval => false']) {
  if (!gate.includes(marker)) fail(`gate missing false-claim marker: ${marker}`);
}
for (const marker of ['blockedMissingQ263Template','blockedMissingEvidenceJsonSchema','blockedMissingDeviceAndAbiEvidence','blockedMissingArtifactEvidence','blockedMissingBridgeEvidence','blockedMissingPerformanceEvidence','blockedMissingSafetyEvidence','blockedUnsafeRuntimeBinding','runtimePrototypeEvidenceIntakeGateStaticReady']) {
...
for (const forbidden of ['camera_android_camerax','google_mlkit_text_recognition','paddleocr','paddle_flutter','paddle_lite','onnxruntime','tesseract','pix2text','flutter_downloader','background_downloader','dio']) {
  if (pubspec.toLowerCase().includes(forbidden)) fail(`Q264 must not add dependency: ${forbidden}`);
}
if (!androidManifest.includes('android.permission.CAMERA')) fail('CAMERA permission must remain');
for (const forbidden of ['FormulaRecognition(', 'runPpFormulaNetSInference', 'setWorkspaceLatexWithoutUserApproval', 'startRealNetworkDownloadInQ264', 'addPaddleRuntimeInQ264', 'bindRuntimeMethodChannelInQ264', 'claimRuntimeSmokePassWithoutAcceptedEvidence']) {
...
]) if (!exists(file)) fail(`Q264 changed-file missing: ${file}`);
if (!changed.includes('gauss_pp_formulanet_s_runtime_prototype_evidence_intake_gate.dart')) fail('changed-files audit missing Q264 Dart file');
if (!readme.includes(phase)) fail('README missing Q264 note');

console.log(JSON.stringify({
  phase,
  status: 'PP_FORMULANET_S_RUNTIME_PROTOTYPE_EVIDENCE_INTAKE_GATE_Q264_STATIC_READY_BUT_REAL_EVIDENCE_RUNTIME_INFERENCE_AND_FLUTTER_RETEST_REQUIRED',
  sourcePhase,
  selectedEngineLabel: q264.selectedEngineLabel,
  evidenceIntakeMode: q264.evidenceIntakeMode,
  runtimePrototypeEvidenceIntakeGateImplemented: q264.runtimePrototypeEvidenceIntakeGateImplemented,
  requiredEvidenceJsonFields: q264.requiredEvidenceJsonFields.length,
  requiredIntakeValidationGates: q264.requiredIntakeValidationGates.length,
  blockedUntilRealEvidence: q264.blockedUntilRealEvidence.length,
  runtimeSmokePassClaimed: q264.runtimeSmokePassClaimed,
  runtimeEvidenceAcceptedWithoutValidation: q264.runtimeEvidenceAcceptedWithoutValidation,
  ocrPassClaimed: q264.ocrPassClaimed,
  realOcrRuntimeAdded: q264.realOcrRuntimeAdded,
  paddleRuntimeAdded: q264.paddleRuntimeAdded,
  paddleOcrDependencyAdded: q264.paddleOcrDependencyAdded,
  nativeBridgeAdded: q264.nativeBridgeAdded,
  jniBindingAdded: q264.jniBindingAdded,
  methodChannelRuntimeBindingAdded: q264.methodChannelRuntimeBindingAdded,
  modelBinaryBundledInBaseApp: q264.modelBinaryBundledInBaseApp,
  productionInferenceAllowed: q264.productionInferenceAllowed,
