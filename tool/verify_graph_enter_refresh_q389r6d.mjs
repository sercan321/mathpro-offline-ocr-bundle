...
const fail = (message) => {
  console.error(`Q389R6D FAIL: ${message}`);
  process.exit(1);
};
...
const test = read('test/q389r6d_graph_enter_refresh_contract_test.dart');
...
expect(test.includes('Q389R6D graph Enter refresh contract'), 'Contract test must exist.');
...
console.log('Q389R6D PASS: visible graph Enter refresh is wired without keyboard/MathLive/OCR/workspace red-line mutations.');
