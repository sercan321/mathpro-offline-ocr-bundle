...
if (process.cwd()) runRetiredHistoricalVerifier({ verifier: 'tool/verify_mathlive_analyze_test_contract_repair_v172_q169r1.mjs', retiredPhase: 'verify_mathlive_analyze_test_contract_repair_v172_q169r1', reason: 'superseded by Q197/Q203 production-surface architecture and Q204R2 full test-suite migration; old diagnostic/fallback/court markers are no longer allowed in active production source or tests' });
...
const files = [
  'lib/features/mathlive/mathlive_analyze_test_contract_repair_policy.dart',
  'lib/features/mathlive/mathlive_main_editor_surface.dart',
  'test/v172_q169r1_mathlive_analyze_test_contract_repair_test.dart',
  'docs/audit/V172_Q169R1_MATHLIVE_ANALYZE_TEST_CONTRACT_REPAIR_AUDIT.md',
  'docs/audit/V172_Q169R1_MATHLIVE_ANALYZE_TEST_CONTRACT_REPAIR_CHANGED_FILES.md'
];
...
requireContains('policy', policy, "phase = 'V172-Q169R1'");
requireContains('policy', policy, 'unusedSurfaceImportRemoved = true');
requireContains('policy', policy, 'q163DiagnosticCompatibilityMarkerPreserved = true');
requireContains('policy', policy, 'activeQ169DiagnosticMarkerPreserved = true');
requireContains('policy', policy, 'runtimeCommandSemanticsChanged = false');
...
requireContains('surface', surface, "analyzerTestContractRepairPhase = 'V172-Q169R1'");
...
requireContains('manifest', manifest, 'v172Q169R1AnalyzeTestContractRepair');
...
if (failures.length) {
  console.error('MATHLIVE_ANALYZE_TEST_CONTRACT_REPAIR_Q169R1_FAILED');
  for (const failure of failures) console.error(' - ' + failure);
  process.exit(1);
}
console.log('MATHLIVE_ANALYZE_TEST_CONTRACT_REPAIR_PACKAGE_READY_BUT_REAL_DEVICE_PAINT_EVIDENCE_REQUIRED');
