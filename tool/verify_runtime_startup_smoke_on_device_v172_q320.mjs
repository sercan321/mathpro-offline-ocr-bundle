...
const phase = 'V172-Q320-RUNTIME-STARTUP-SMOKE-ON-DEVICE';
const sourcePhase = 'V172-Q319-VERIFIED-ARTIFACT-LOAD-SMOKE-ON-DEVICE';
...
if (![phase, q321SuccessorPhase, q322SuccessorPhase, q323SuccessorPhase, q324SuccessorPhase, q325SuccessorPhase, q326SuccessorPhase, q327SuccessorPhase, q328SuccessorPhase, q329SuccessorPhase, q330SuccessorPhase, q331SuccessorPhase, q332SuccessorPhase, q333SuccessorPhase, q334SuccessorPhase, q335SuccessorPhase, q336SuccessorPhase, q337SuccessorPhase, q338SuccessorPhase, q339SuccessorPhase, q340SuccessorPhase, q341SuccessorPhase, q342SuccessorPhase].includes(manifest.cameraOcrLatestPhase)) fail(`cameraOcrLatestPhase must be Q320 or Q321 successor, got ${manifest.cameraOcrLatestPhase}`);
if (![phase, q321SuccessorPhase, q322SuccessorPhase, q323SuccessorPhase, q324SuccessorPhase, q325SuccessorPhase, q326SuccessorPhase, q327SuccessorPhase, q328SuccessorPhase, q329SuccessorPhase, q330SuccessorPhase, q331SuccessorPhase, q332SuccessorPhase, q333SuccessorPhase, q334SuccessorPhase, q335SuccessorPhase, q336SuccessorPhase, q337SuccessorPhase, q338SuccessorPhase, q339SuccessorPhase, q340SuccessorPhase, q341SuccessorPhase, q342SuccessorPhase].includes(manifest.activeProductDevelopmentLatestPhase)) fail(`activeProductDevelopmentLatestPhase must be Q320 or Q321 successor, got ${manifest.activeProductDevelopmentLatestPhase}`);
if (manifest.runtimeStartupSmokeOnDeviceLatestPhase !== phase) fail('latest Q320 marker missing');
...
if (manifest.verifiedArtifactLoadSmokeOnDeviceLatestPhase !== sourcePhase) fail('Q319 source latest marker missing');
if (!manifest.v172Q319VerifiedArtifactLoadSmokeOnDevice) fail('Q319 source envelope missing');
const q320 = manifest.v172Q320RuntimeStartupSmokeOnDevice;
if (!q320) fail('Q320 manifest envelope missing');
if (q320.phase !== phase) fail('Q320 manifest phase mismatch');
if (q320.sourcePhase !== sourcePhase) fail('Q320 source phase mismatch');
if (q320.nativeChannelName !== 'mathpro/ocr_runtime_bridge') fail('Q320 native channel mismatch');
if (q320.runtimeStartupSmokeMethod !== 'runtimeStartupSmoke') fail('Q320 method mismatch');
if (q320.runtimeStartupSmokeStatus !== 'RUNTIME_STARTUP_SMOKE_BLOCKED_NO_RUNTIME_DEPENDENCY_DEFAULT_OFF') fail('Q320 status mismatch');
if (q320.expectedInferencePdiparamsSha256 !== expectedSha256) fail('Q320 expected SHA mismatch');
if (q320.expectedInferencePdiparamsSizeBytes !== 231675001) fail('Q320 artifact size evidence mismatch');
for (const key of [
  ...
  'runtimeStartupSmokeMethodAdded',
  'mainActivityChanged',
  'protectedMainActivityChangeDeclared',
  'methodChannelBindingExtended'
]) {
  if (q320[key] !== true) fail(`Q320 ${key} must be true`);
}
for (const key of [
  'q315RuntimeAccepted',
  ...
  if (q320[key] !== false) fail(`Q320 ${key} must remain false`);
}
for (const field of ['phase','sourcePhase','method','channel','status','artifactPath','artifactExists','artifactSizeBytes','expectedArtifactSha256','actualArtifactSha256','sha256MatchesExpected','verifiedArtifactLoadSmokePassed','runtimeStartupBlockedReason','runtimeDependencyAdded','runtimeStartupAttempted','runtimeStartupExecuted','runtimeStartupSmokePassed','modelLoaderAvailable','modelLoaded','dummyRuntimeCallExecuted','imageToLatexInferenceExecuted','editableMathLiveReviewOpened','workspaceImportExecuted','ocrPassClaimed']) {
  if (!q320.runtimeStartupSmokeEnvelopeFields.includes(field)) fail(`Q320 envelope field missing: ${field}`);
}
for (const item of ['no-paddle-onnx-tflite-runtime-dependency-in-q320','no-jni-native-model-loader-in-q320','no-runtime-startup-attempt-without-selected-runtime-dependency-in-q320','no-dummy-runtime-call-or-image-to-latex-in-q320','no-ocr-pass-or-android-real-device-pass-without-real-run-log']) {
  if (!q320.q320HardBlocks.includes(item)) fail(`Q320 hard block missing: ${item}`);
}
for (const item of ['q320-flutter-run-device-log-shows-runtimeStartupSmoke-method-response','q320-response-keeps-runtimeStartupExecuted-false-with-no-runtime-dependency','runtime-dependency-selection-approved-before-dummy-runtime-call']) {
  if (!q320.q321EntryCriteria.includes(item)) fail(`Q321 entry criterion missing: ${item}`);
}
...
for (const file of [
  ...
  if (!fs.existsSync(file)) fail(`Q320 file missing: ${file}`);
}
const policy = read('lib/features/camera/gauss_runtime_startup_smoke_on_device_q320_policy.dart');
for (const marker of [phase, sourcePhase, 'runtimeStartupSmoke', 'RUNTIME_STARTUP_SMOKE_BLOCKED_NO_RUNTIME_DEPENDENCY_DEFAULT_OFF', 'gauss.ppFormulaNetS.runtimeStartupSmoke.defaultOff', expectedSha256]) {
  if (!policy.includes(marker)) fail(`Q320 policy marker missing: ${marker}`);
}
const impl = read('lib/features/camera/gauss_runtime_startup_smoke_on_device_q320.dart');
if (!impl.includes('return const GaussRuntimeStartupSmokeOnDeviceQ320Result(')) fail('Q320 evaluate must return const result envelope');
...
for (const marker of [
  ...
  if (!mainActivity.includes(marker)) fail(`Q320 MainActivity marker missing: ${marker}`);
}
for (const forbidden of ['PaddleOCR','PaddleLite','loadModel(','runInference(','imageToLatex(','JNIEnv','OkHttpClient','DownloadManager']) {
  if (mainActivity.includes(forbidden)) fail(`forbidden runtime/download marker in Q320 MainActivity: ${forbidden}`);
}

const changed = read('docs/audit/V172_Q320_CHANGED_FILES.md');
if (!changed.includes('Protected Runtime File Intentionally Modified')) fail('Q320 changed files must declare intentional protected MainActivity change');
if (!changed.includes('android/app/src/main/kotlin/com/mathpro/flutter/MainActivity.kt')) fail('Q320 changed files must list MainActivity');
if (!changed.includes('default-off runtime startup smoke preflight method only')) fail('Q320 changed files must explain MainActivity reason');
const audit = read('docs/audit/V172_Q320_RUNTIME_STARTUP_SMOKE_ON_DEVICE_AUDIT.md');
if (!audit.includes('Runtime accepted: `false`')) fail('Q320 audit must keep runtime acceptance false');
if (!audit.includes('RUNTIME_STARTUP_SMOKE_BLOCKED_NO_RUNTIME_DEPENDENCY_DEFAULT_OFF')) fail('Q320 audit must disclose blocked startup status');
const readme = read('README.md');
if (!readme.includes('V172-Q320')) fail('README must mention Q320');
if (!readme.includes('RUNTIME_STARTUP_SMOKE_BLOCKED_NO_RUNTIME_DEPENDENCY_DEFAULT_OFF')) fail('README must disclose Q320 smoke status');
...
for (const forbidden of ['paddleocr','paddle_lite','paddle-lite','onnxruntime','tflite_flutter','workmanager','background_downloader','dio','http:','flutter_downloader']) {
  if (pubspec.includes(forbidden)) fail(`forbidden runtime/download dependency added in Q320: ${forbidden}`);
}
...
for (const forbidden of ['READ_EXTERNAL_STORAGE','MANAGE_EXTERNAL_STORAGE','FOREGROUND_SERVICE_DATA_SYNC']) {
  if (androidManifest.includes(forbidden)) fail(`forbidden Android permission added in Q320: ${forbidden}`);
}
...
console.log(JSON.stringify({
  phase,
  status: 'RUNTIME_STARTUP_SMOKE_BOUND_DEFAULT_OFF_NO_RUNTIME_DEPENDENCY',
  sourcePhase,
  nativeChannelName: q320.nativeChannelName,
  runtimeStartupSmokeMethod: q320.runtimeStartupSmokeMethod,
  runtimeDependencyAdded: q320.runtimeDependencyAdded,
  runtimeStartupAttempted: q320.runtimeStartupAttempted,
  runtimeStartupExecuted: q320.runtimeStartupExecuted,
  runtimeStartupSmokePassed: q320.runtimeStartupSmokePassed,
  modelArtifactAcceptedForRuntime: q320.modelArtifactAcceptedForRuntime,
  realImageToLatexInferenceExecuted: q320.realImageToLatexInferenceExecuted,
  ocrPassClaimed: q320.ocrPassClaimed,
  androidRealDevicePassClaimed: q320.androidRealDevicePassClaimed
}, null, 2));
